...
def template_data(request, app_uid=None):
    ...
    for plugin_class in plugins4menus:
        ...
        menu.url = plugin_class.ORQUESTRA_URL if hasattr(plugin_class, 'ORQUESTRA_URL') else '/app/{0}/'.format(
            menu.uid)
        menu.target = 'target={0}'.format(plugin_class.ORQUESTRA_TARGET) if hasattr(plugin_class,
                                                                                    'ORQUESTRA_TARGET') else ''
